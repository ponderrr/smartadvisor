from sqlalchemy import Table, Column, String, Integer, Boolean, DateTime, ForeignKey, Float, Text
...
class RecommendationQuestions(Base):
    ...
    question_order = Column(Integer)  # Order of the question in the recommendation process
    created_at = Column(DateTime, default=datetime.utcnow)
...
movie_genres = Table(
    "movie_genres",
    Base.metadata,
    Column("movie_recommendation_id", String(36), ForeignKey("movie_recommendations.id"), primary_key=True),
...
book_genres = Table(
    "book_genres",
    Base.metadata,
    Column("book_recommendation_id", String(36), ForeignKey("book_recommendations.id"), primary_key=True),
    Column("genre", String, ForeignKey("genres.name"), primary_key=True)
)
